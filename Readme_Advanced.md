...
must be NVIDIA Corporation by name, with a chain that is clean when built again from Microsoft's own trusted root list and the certificates the signature carries alone, so a root anyone added to the ordinary Windows stores does not count. That build reads only what the machine holds already, so Microsoft's copy of the root NVIDIA chains to has to be there, which Windows keeps up to date on its own. A file that fails either stops the session before NGX is so much as initialised. One that passes is then asked what its version resource calls its product:
